...
import { OLD_SCRIPT, NEW_SCRIPT, versions, type VersionItem } from './mock'
import './versions.css'

// Groovy highlight style (dark palette matching #0F172A) — 1:1 from demo.
...
function Versions() {
  ...
  const statusTagClass = (s: VersionItem['status'], current?: boolean) =>
    current ? 'current' : s
              ...
              {versions.map((item) => (
                ...
                >
                  <div className="version-row">
                    ...
                    <span className={`version-status-tag ${statusTagClass(item.status, item.current)}`}>
                      {item.current ? 'PUBLISHED' : item.status.toUpperCase()}
                    </span>
                  </div>
                  <div className="version-meta">{item.meta}</div>
                </div>
              ))}
            </div>
