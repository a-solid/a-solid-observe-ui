...
import { sourceGroups, type SourceKind, type ActionType, type Subscription } from './mock'
...
function SubCard({ s }: { s: Subscription }) {
  const forkLabel = s.forkTo ? `分叉到 ${s.forkTo}` : null
  return (
    <Link to="/subscriptions/order-event/edit" className={`sub-card ${s.source}`}>
      <div className="sub-card-head">
        ...
        <div className="action-status">
          <span className={`action-pill ${s.action}`}>{ACTION_ICON[s.action]}{s.action.toUpperCase()}</span>
          <span className={`status-pill ${s.status}`}><span className="dot" />{s.status.toUpperCase()}</span>
        </div>
      </div>
      ...
      <div className="pipeline-bindings">
        <div className="bindings-label">
          {BIND_ICON}
          Pipeline 绑定
          <span className="count">{s.bindings.length}{s.forkTo ? ' · 分叉' : ''}</span>
        </div>
      ...
      <div className="sub-card-foot">
        ...
        <div className="sub-card-actions">
          <span className="icon-btn" title="编辑">{EDIT_ICON}</span>
        </div>
      </div>
    </Link>
  )
      ...
      <main className="page subscriptions-page">
        <div className="page-header">
          <div>
            ...
            <p className="page-subtitle"><span className="num">8</span> 个 subscription · CDC / CRON / API 三类来源</p>
          </div>
        ...
        {sourceGroups.map((g) => (
          <div className={`source-group ${g.source}`} key={g.source}>
        ...
        <div className="list-footer">
          共 <span className="num">8</span> 个 subscription · 跨 <span className="num">14</span> 个 pipeline 绑定
        </div>
      </main>
    </>
  )
